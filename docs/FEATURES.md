...
| ID | Feature | Status | Owner | Evidence | Last Touched | Blockers | Notes |
|----|---------|--------|-------|----------|--------------|----------|-------|
| P2.5-01 | Slash command dispatch in `telegram_handlers.py` | BUILT | Claude | 8 tests green | 2026-04-25 | — | `CommandHandler` for `/briefing`, `/status`, `/help`, `/health`, `/budget`. Unknown slash → friendly "try plain English" reply. Tests: 5+ |
| P2.5-02 | `BriefingAgent.execute()` pulls real integrations | BUILT | Claude | 8 tests green | 2026-04-25 | — | LOCAL_BRAIN is now primary composer; Gemini fallback preserved (HC-09). `_build_local_context()` truncates: 5 events / 5×2 emails / 5 tasks. |
| P2.5-03 | `ContextBuilder` real-data path | BUILT | Claude | 8 tests green | 2026-04-25 | — | Intent-driven pre-fetch for weather/schedule/email/finance → `AgentContext.system_context`. Keyword regex + intent type triggers. HC-09 graceful degradation. |
| P2.5-04 | Pre-fetch + inject pattern for local tier (no native tool-call) | BUILT | Claude | 5 tests green | 2026-04-25 | depends P2.5-03 | `_inject_system_context()` → `[CONTEXT]\n{json}\n[/CONTEXT]` prepended before `_local_call`. `docs/code_conventions.md` "Tool-use on local tier" section added. |
| P2.5-05 | Redis env drift fix (P0-13 followup) | SHIPPED | Claude | /health redis=connected | 2026-04-25 | — | `/opt/tars/deploy/node1/.env` REDIS_URL updated 192.168.12.201:6379 → 100.94.4.103:6379. Restarted tars-backend; health green. |
| P2.5-06 | Persona prefix loader (pull-forward P5-02 partial) | BUILT | Claude | 6 tests green | 2026-04-25 | — | `shared/persona.py::load_persona()` + `@lru_cache`. `data/persona/local.md` holds persona text. Engine imports via `load_persona("local")`. |
| P2.5-07 | Per-session ritual: deploy-drift + Redis health gates | SHIPPED | Claude | Doc merged | 2026-04-25 | — | `CLAUDE_PHASE_HANDOUT.md` ritual updated: deploy-drift check + Redis-status assertion added. Catches stale-code + Redis-IP regressions. |
...
| P3-03 | `integrations/qdrant_client.py` | BUILT | Claude | `backend/src/integrations/qdrant_client.py` + `backend/tests/test_qdrant_client.py` (18 tests) | 2026-04-28 | depends P3-01 | Async wrapper over `AsyncQdrantClient`; `ensure_collection`/`upsert`/`search`/`delete`/`count`/`close`; HC-09 typed `QdrantError`; structlog on every op. |
...
| P3-07 | `sensors/wiki_watcher.py` (watchdog) | BUILT | Claude | `backend/src/sensors/wiki_watcher.py` + tests (15 tests). watchdog>=4.0 dep added. | 2026-04-28 | depends P3-03..4, P3-08 | Filesystem observer on wiki/. Debounce 500ms. create/modify→ingest_file; delete→qdrant+wiki_index purge. HC-09 fail-soft. |
...
| P3.5-02 | Postgres `world_state` table + monthly partitioning (was P4-09) | BUILT | Claude | 13 tests green (4 PG-only) | 2026-04-25 | — | Alembic 007 migration: native PG `PARTITION BY RANGE (ts)`, composite PK `(id, ts)`, index `(sensor, ts DESC)`, 24 pre-created monthly partitions (2026-01..2027-12) + default catch-all. partman extension swap deferred (postgres:16-alpine has no partman); cron-driven partition rotation comes in P4. |
| P3.5-03 | Weather sensor (was P4-07) | BUILT | Claude | 4 tests green | 2026-04-26 | depends P3.5-01..02 | 15-min cadence. Wraps existing `weather_client.py`. Writes to world_state + publishes pub/sub. |
| P3.5-04 | HealthKit sensor migration (was P4-06) | BUILT | Claude | 4 tests green | 2026-04-26 | depends P3.5-01..02 | Existing health_data ingest → adapt to BaseSensor pattern + world_state writes. |
| P3.5-05 | Tailscale presence sensor (was P4-08) | BUILT | Claude | 4 tests green | 2026-04-26 | depends P3.5-01..02 | Tailscale API. 2-min cadence. Tracks which devices online. |
| P3.5-06 | Spotify sensor (was P4-04) | BUILT | Claude | 5 tests green | 2026-04-26 | depends P3.5-01..02 | spotipy. 30s cadence. Now-playing track. Fail-soft if token missing. |
| P3.5-07 | `BriefingAgent` + `ContextBuilder` switch to read `world_state` | BUILT | Claude | 24 tests green (8 cache + 12 briefing + 4 context-builder); 1227 total | 2026-04-26 | depends P3.5-03..06 | `orchestrator/world_state_cache.read_fresh_sensor()` reads `world_state` with 3× `min_interval_seconds` freshness window. BriefingAgent `_fetch_weather`/`_fetch_health_data` prefer cache, fall back to integration on miss/stale/raise (HC-09). Added `_fetch_tailscale_presence` + `_fetch_spotify_now_playing` (cache-only, no integration in briefing). ContextBuilder `_pre_fetch_weather` reads cache first. |
...
| P4-13 | `orchestrator/autonomy_budget.py` | TESTED | Claude | `backend/src/orchestrator/autonomy_budget.py` + tests | 2026-04-28 | depends P4-11..12 | BudgetCheck dataclass, READ/WRITE_LOCAL pass-through, WRITE_SELF daily+weekly cap, defensive raise for WRITE_WORLD/INFRA. Engine integration in follow-up; engine wired (autonomy_budget gate before approval flow, WRITE_SELF cap downgrades to approval). |
...
| P4-15 | Trigger: evening_wind_down | BUILT | Claude | `backend/src/orchestrator/triggers/evening_wind_down.py` + tests (20 tests) | 2026-04-28 | depends P4-10 | First concrete Trigger on P4-10. Conditions: home (tailscale-proxy) + evening 18:00-22:30 local + no meeting within 2h. Dedup via audit_log per local day. Notification side-effect deferred. |
